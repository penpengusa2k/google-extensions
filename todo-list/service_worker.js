...
async function openTodoWindow() {
  const url = chrome.runtime.getURL("ui/todo.html");
  const all = await chrome.windows.getAll({ populate: true, windowTypes: ["popup", "normal"] });
  for (const w of all) {
    for (const t of (w.tabs || [])) {
      if (t.url === url) {
...
async function tryToggleTodo(tabId) {
  try {
    await chrome.tabs.sendMessage(tabId, { type: "toggleTodo" });
    return true;
  } catch (error) {
    if (!isMissingReceiverError(error)) {
      console.warn("Failed to toggle todo overlay.", error);
      return false;
    }
  }
  try {
    ...
    console.warn("Failed to inject todo overlay content script.", injectionError);
    return false;
  }
}

async function toggleTodoForActiveTab() {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!tab) return;
  if (isSkippableUrl(tab.url)) {
  ...
  const toggled = await tryToggleTodo(tab.id);
  if (!toggled) {
    await openTodoWindow();
  }
}
...
chrome.commands.onCommand.addListener(async (cmd) => {
  ...
  else if (cmd === "open-todo-list") await toggleTodoForActiveTab();
});
